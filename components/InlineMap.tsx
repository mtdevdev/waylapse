...
import { Play, Pause, RotateCcw, MapPin, Clock, Route as RouteIcon, ArrowDown, Music, User, Video, Download, X } from 'lucide-react';
...
interface Props {
  ...
  onExportStart?: () => void;
  onExportEnd?: () => void;
  triggerExport?: boolean; // Prop to trigger export from parent (Settings)
}
...
const InlineMap: React.FC<Props> = ({ route, config, t, language, onExportStart, onExportEnd, triggerExport }) => {
  ...
  // Hack to trigger full intro restart
  ...
  // Listen for parent export trigger
  useEffect(() => {
      if (triggerExport) {
  ...
  useEffect(() => {
    if (activePolylineRef.current) activePolylineRef.current.setStyle({ weight: config.lineWidth, color: config.lineColor });
    if (markerRef.current) markerRef.current.setStyle({ fillColor: config.lineColor });
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
        const map = L.map(mapContainerRef.current, {
            zoomControl: false, attributionControl: false, dragging: false, scrollWheelZoom: false, doubleClickZoom: false, boxZoom: false, keyboard: false, zoomAnimation: true, fadeAnimation: true, inertia: true
        });
        const tileLayer = L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
            maxZoom: 20, subdomains: 'abcd', opacity: 1, crossOrigin: 'anonymous'
        });
        tileLayerRef.current = tileLayer;
        if (configRef.current.showMap) tileLayer.addTo(map);
  ...
  useEffect(() => {
    ...
    const runCinematicIntro = async () => {
        ...
        activePolylineRef.current = L.polyline([], { color: configRef.current.lineColor, weight: configRef.current.lineWidth, opacity: 1, lineCap: 'round', lineJoin: 'round' }).addTo(map);
        markerRef.current = L.circleMarker(path[0], { radius: 3, fillColor: configRef.current.lineColor, fillOpacity: 0, stroke: false }).addTo(map);
        ...
        labelsLayerRef.current.eachLayer((layer) => { if (layer instanceof L.Marker) { const el = layer.getElement(); if (el) el.style.opacity = '0'; } });
        
        map.flyToBounds(L.latLngBounds(path), { 
            paddingTopLeft: [40, 40],
            paddingBottomRight: [40, 160],
            animate: true, 
            duration: 3.5 * speedMult,
            easeLinearity: 0.2
        });
        ...
        introLayersRef.current.eachLayer((layer) => { if (layer instanceof L.Marker) { const el = layer.getElement(); if (el) el.classList.add('opacity-0'); } });
        ...
        if (configRef.current.animationType !== 'OVERVIEW') {
            const startZoom = configRef.current.animationType === 'FOCUS' ? 16 : 14;
            map.flyTo(path[0], startZoom, { duration: 2.0 * speedMult, easeLinearity: 0.2 });
            zoomPromise = delay(2200);
        }

        await new Promise(r => setTimeout(r, 1100));
        introLayersRef.current.clearLayers();
        labelsLayerRef.current.eachLayer((layer) => { if (layer instanceof L.Marker) { const el = layer.getElement(); if (el) el.style.opacity = '1'; } });
        await zoomPromise;
        ...
        // SYNC POINT: Intro is done, now we enable playback
        setIntroStage(IntroStage.COMPLETED);
        
        // Reset timing variables one last time to be sure
        startTimeRef.current = null; 
        pausedTimeRef.current = 0;
        progressRef.current = 0;
  ...
  useEffect(() => {
    ...
    if (!isPlaying) {
        ...
        if (isRendering && progressRef.current >= 1) {
            handleStopRecording();
        }
        return;
    }
    ...
    const animate = (timestamp: number) => {
        ...
        if (progressBarRef.current) progressBarRef.current.style.width = `${rawProgress * 100}%`;
  ...
  }, [isPlaying, route, introStage, isRendering]);
  ...
  const handleInteraction = () => {
      if (isRendering) return; // Block interaction during recording
      if (introStage !== IntroStage.COMPLETED) return;
      setShowControls(true);
      if (idleTimeoutRef.current) clearTimeout(idleTimeoutRef.current);
      if (isPlaying) idleTimeoutRef.current = window.setTimeout(() => setShowControls(false), 2500);
  };
  ...
  const togglePlay = () => {
    if (isPlaying) {
        ...
        if (progressRef.current >= 1) {
            ...
            pausedTimeRef.current += (now - pauseStart);
            setIsPlaying(true);
            idleTimeoutRef.current = window.setTimeout(() => setShowControls(false), 2500);
        }
    }
  ...
  const restart = () => {
      // Logic handled in useEffect dependent on restartTrigger
  ...
  const handleStartRecording = async () => {
      try {
          ...
          mediaRecorder.onstop = () => {
              ...
              // Only download if we have data (user didn't cancel immediately)
          ...
          // Native Cancel Listener (Stop Sharing bar)
          stream.getVideoTracks()[0].onended = () => {
             if (mediaRecorder.state !== 'inactive') {
                 mediaRecorder.stop();
                 // This effectively cancels the logic in onstop but we treat it as a stop
                 setIsRendering(false);
                 onExportEnd?.();
             }
          };
          ...
          onExportStart?.(); // Notify parent to lock UI
  ...
  return (
    <div 
        className={`w-full h-full relative bg-black overflow-hidden ${isRendering ? 'cursor-none' : ''}`}
    ...
    >
        ...
        {config.musicEnabled && config.socialEnabled && (
            <audio ref={audioRef} src={config.customAudioUrl || undefined} loop className="hidden" />
        )}

        {/* Map Wrapper */}
        <div className="w-full h-full absolute inset-0 z-0" style={{ filter: `brightness(${config.brightness}) contrast(${config.contrast})` }}>
            <div ref={mapContainerRef} className="w-full h-full outline-none" style={{ backgroundColor: '#000000' }} />
        </div>

        {/* --- Social Overlay Elements (Intro) --- */}
                         ...
                         <div className="bg-black/30 backdrop-blur-md border border-white/10 px-4 py-1.5 rounded-full flex items-center gap-2 shadow-lg max-w-[160px] md:max-w-[200px] overflow-hidden">
                            ...
                            <div className="text-[10px] font-medium text-white whitespace-nowrap overflow-hidden">
                                 <div className="animate-marquee inline-block">
                                    {config.customAudioUrl ? `${config.customAudioName || t.customAudio} • ${t.playingNow}` : t.noAudio}
                                 </div>
                            </div>
                        </div>
                    </div>
                     ...
                     <div className={`absolute bottom-4 left-4 scale-90 origin-bottom-left md:bottom-6 md:left-8 md:scale-100 flex flex-col items-start gap-2 animate-slide-in-right ${introStage < IntroStage.FADE_OUT_STATS ? 'pointer-events-auto' : 'pointer-events-none'}`}>
                        <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-3 pr-5 rounded-2xl flex items-center gap-3 shadow-2xl hover:bg-black/60 transition-colors">
                            <div className="w-10 h-10 rounded-full bg-neutral-800 flex items-center justify-center border border-white/10 shadow-inner overflow-hidden">
                                {config.userImage ? <img src={config.userImage} alt="User" className="w-full h-full object-cover" /> : <User size={20} className="text-white/80" />}
                            </div>
                            <div className={`flex flex-col ${!displayHandle ? 'hidden' : ''}`}>
                                ...
                                <div className="text-sm font-bold text-white flex items-center gap-1.5">@{displayHandle}</div>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="max-w-lg w-full flex flex-col items-center">
                    <div className="space-y-2 animate-slide-in-down" style={{animationDelay: '300ms'}}>
                         ...
                         <div className="flex flex-col items-center">
                            {route.startLabel.subtitle && <span className="text-sm text-neutral-400 font-medium">{route.startLabel.subtitle}</span>}
                    ...
                    <div className="space-y-3 animate-slide-in-up opacity-0" style={{animationDelay: '2200ms', animationFillMode: 'forwards'}}>
                        ...
                        <div className="flex flex-col items-center">
                             ...
                             {route.endLabel.subtitle && <span className="text-sm md:text-base text-neutral-400 font-medium mt-1">{route.endLabel.subtitle}</span>}
                        </div>
                    </div>
                    ...
                    <div className="w-full mt-12 mb-4 relative">
                        ...
                        <div className="grid grid-cols-2 gap-12 py-8 opacity-0 animate-fade-in-up" style={{animationDelay: '3800ms', animationFillMode: 'forwards'}}>
                            <div className="space-y-2">
                                 ...
                                 <div className="text-4xl font-bold text-white"><CountUp value={formattedDuration} delay={4200} /></div>
                            </div>
                            <div className="space-y-2">
                                 <div className="flex items-center justify-center gap-2 text-neutral-500">
                                    <RouteIcon size={16} />
                                 ...
                                 <div className="text-4xl font-bold text-white"><CountUp value={formattedDistance} delay={4200} /></div>
                            </div>
                        </div>
        ...
        {/* Persistent Route Summary & Download Button (Main View) - Top Right */}
        {introStage === IntroStage.COMPLETED && config.showStats && (
             <div className={`absolute top-4 right-4 md:top-6 md:right-6 z-20 transition-opacity duration-300 ${isRendering ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <div className="bg-black/60 backdrop-blur-md border border-white/10 p-5 rounded-2xl flex flex-col gap-4 min-w-[280px] max-w-[340px] shadow-2xl scale-90 origin-top-right md:scale-100">
                    <div>
                        ...
                        {route.endLabel.subtitle && <div className="text-sm text-neutral-400 mt-1 truncate">{route.endLabel.subtitle}</div>}
                    </div>
                    ...
                    {/* End Screen Video Download CTA */}
                    {progressRef.current >= 1 && (
                        <div className="pt-2 animate-fade-in">
                            <button 
                                onClick={handleStartRecording}
                                className="w-full py-2 bg-white text-black font-bold text-xs uppercase tracking-widest rounded-lg hover:bg-neutral-200 transition-colors flex items-center justify-center gap-2"
                            >
                                <Video size={14} />
                                {t.exportVideo}
                            </button>
        ...
        {/* Playback Controls - Bottom Center (Hidden during rendering) */}
        {introStage === IntroStage.COMPLETED && !isRendering && (
            <div className={`absolute bottom-8 md:bottom-12 left-0 right-0 z-20 flex flex-col items-center gap-6 md:gap-8 px-6 pointer-events-none pb-[env(safe-area-inset-bottom)] transition-all duration-700 ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
                ...
                <div className="w-full max-w-md h-1.5 bg-white/10 rounded-full overflow-hidden backdrop-blur-sm">
                    <div ref={progressBarRef} className="h-full bg-white shadow-[0_0_20px_rgba(255,255,255,1)]" style={{ width: '0%' }}></div>
                </div>
            </div>
        )}

        <style dangerouslySetInnerHTML={{ __html: `
            @keyframes intro-pulse { 0% { transform: scale(1); opacity: 0.6; } 100% { transform: scale(2.0); opacity: 0; } }
            @keyframes intro-pulse-large { 0% { transform: scale(1); opacity: 0.5; } 100% { transform: scale(2.5); opacity: 0; } }
            ...
            @keyframes revealArrow { from { opacity: 0; transform: scaleY(0); } to { opacity: 1; transform: scaleY(1); } }
